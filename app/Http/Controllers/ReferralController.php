...
class ReferralController extends Controller
{
    public static function update(Request $request){
        ...
        $referral->voucher_id = $request->referrer['voucher_id'] == ReferralEnums::$TYPE['voucher'] ? $request->referrer['voucher_id'] : null;
        $referral->meta = null;
        $referral->status = ReferralEnums::$STATUS['active'];
        ...
        $referee->voucher_id = $request->referee['voucher_id'] == ReferralEnums::$TYPE['voucher'] ? $request->referee['voucher_id'] : null;
        $referee->meta = null;
        $referee->status = ReferralEnums::$STATUS['active'];
        $referee->referral_role = ReferralEnums::$ROLE['referee'];
                    ...
                    if ($rule->refferal_role == ReferralEnums::$ROLES['referer']) {
                        switch($rule->reward_type){
                            case ReferralEnums::$TYPE["points"]:
                                ...
                                ReferralHistory::where("status",ReferralHistoryEnums::$STATUS['reward_pending'])
                                    ->where("trigger_at",$trigger_type)
                                    ->first();
                            break;
