...
class VendorWebController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $booking_live = Booking::whereIn('status', [BookingEnums::$STATUS['biding'], BookingEnums::$STATUS['rebiding']])
            ->whereIn("id", Bid::where("organization_id", Session::get('organization_id'))->where("status", BidEnums::$STATUS['active'])->pluck("booking_id"))
            ->limit(3)
            ->get();
...
//            $last_week[$key] = $date->format('d M');
        ...
        $order_dist = Booking::select('status', DB::raw("count(*) AS count"))->where('organization_id', Session::get('organization_id'))
            ->groupBy('status')
            ->get();
    ...
    {
        $count_booking = Bid::where(['organization_id' => Session::get('organization_id')])->whereIn('status', [BidEnums::$STATUS['active']])->whereIn('booking_id', Booking::whereIn('status', [BookingEnums::$STATUS['biding'], BookingEnums::$STATUS['rebiding']])->pluck('id'))->count();
        $participated_booking = Bid::where(['organization_id' => Session::get('organization_id')])->whereIn('status', [BidEnums::$STATUS['bid_submitted'], BidEnums::$STATUS['lost']])->whereIn('booking_id', Booking::whereNotIn('status', [BookingEnums::$STATUS['bounced'], BookingEnums::$STATUS['hold'], BookingEnums::$STATUS['in_progress'], BookingEnums::$STATUS['cancel_request'], BookingEnums::$STATUS['cancelled']])->pluck('id'))->count();
        $schedul_booking = Bid::where(['organization_id' => Session::get('organization_id')])->whereIn('status', [BidEnums::$STATUS['won']])->whereIn('booking_id', Booking::where("organization_id", Session::get('organization_id'))->whereNotIn('status', [BookingEnums::$STATUS['bounced'], BookingEnums::$STATUS['cancel_request'], BookingEnums::$STATUS['completed'], BookingEnums::$STATUS['cancelled']])->pluck('id'))->count();
        $save_booking = Bid::where(['organization_id' => Session::get('organization_id'), 'bookmarked' => CommonEnums::$YES])->where("status", BidEnums::$STATUS['active'])->whereIn('booking_id', Booking::whereNotIn('status', [BookingEnums::$STATUS['bounced'], BookingEnums::$STATUS['hold'], BookingEnums::$STATUS['in_progress'], BookingEnums::$STATUS['cancel_request'], BookingEnums::$STATUS['cancelled']])->pluck('id'))->count();

        $past_booking = Bid::where(['organization_id' => Session::get('organization_id'), 'status' => BidEnums::$STATUS['won']])->whereIn('booking_id', Booking::where("organization_id", Session::get('organization_id'))->whereIn('status', [BookingEnums::$STATUS['completed'], BookingEnums::$STATUS['cancelled'], BookingEnums::$STATUS['cancel_request']])->pluck('id'))->count();

        // return [$count_booking, $participated_booking, $schedul_booking, $save_booking, $past_booking];
    ...
    public function userAdd(Request $request)
    {
        ...
        $roles = Vendor::where("id", Session::get('account')['id'])->first();
        ...
        return view('vendor-panel.user.add_user', ['id' => $request->id, 'roles' => $users, 'user_role' => $roles, 'branches' => $branch]);
    }
