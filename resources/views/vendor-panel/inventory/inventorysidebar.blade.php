            ...
            <div class="d-flex  row justify-content-between p-8">
                ...
                <div class="col-sm-12 p-0">
                    @foreach($service_types as $service_type)
                        <label style="font-size: 14px; font-weight: 700;">{{$service_type->service->name}}</label>
                                ...
                                @foreach($inventories->prices as $inventory)
                                    @if($service_type->service->id == $inventory->service_type)
                                        <tr class="tb-border">
                                            ...
                                            <td class="text-center">@if($inventory->mp_premium)₹ {{$inventory->mp_premium}} @else NA @endif</td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
