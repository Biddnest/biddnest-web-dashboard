@extends('vendor-panel.layouts.frame')
@section('title') Manage Inventory @endsection
@section('body')
    <div class="main-content grey-bg" data-barba="container" data-barba-namespace="inventorymgt">
        <div class="d-flex  flex-row justify-content-between vertical-center">
            <h3 class="page-head text-left p-4 f-20 theme-text">Inventory Management</h3>
            <div class="mr-20">
                <a href="inventory-create-item.html">
                    <button class="btn theme-bg white-text"><i class="fa fa-plus p-1"
                                                               aria-hidden="true"></i>CREATE New Item</button>
                </a>

            </div>
        </div>
        <div class="d-flex  flex-row justify-content-between">
            <div class="page-head text-left  pt-0 pb-0 p-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item active" aria-current="page">Inventory Management</li>
                    </ol>
                </nav>


            </div>

        </div>

    <!-- Dashboard cards -->


    <div class="d-flex flex-row justify-content-center Dashboard-lcards ">
        <div class="col-lg-12">
            <div class="card h-auto p-0 pt-20">
                <div class="card-head right text-left  p-15 pt-20 pb-0 row justify-content-between d-felx">
                    <div class="col-lg-8">
                        <h3 class="f-18  theme-text">
                            Selelect Category
                        </h3>
                    </div>
                    <!-- <div class="col-lg-4 margin-topneg-15">
                        <div class="form-input">

                          <select id="ban-type" class="form-control br-5">
                            <option>All</option>
                            <option> Furniture</option>
                            <option>Vehicle</option>
                          </select>



                          <span class="error-message">Please enter a valid banner type</span>
                        </div>
                      </div>   -->

                </div>
                <ul class="nav nav-tabs p-15 secondg-bg pt-0 pb-0" id="myTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active p-15" id="live-tab" data-toggle="tab" href="#live" role="tab" aria-controls="home" aria-selected="true">All</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link p-15" id="past-tab" data-toggle="tab" href="#past" role="tab" aria-controls="profile" aria-selected="false">Furniture</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link  p-15" id="driver-tab" data-toggle="tab" href="#drivers" role="tab" aria-controls="home" aria-selected="false">Electronics</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link  p-15" id="driver-tab" data-toggle="tab" href="#electrical" role="tab" aria-controls="home" aria-selected="false">Electrical</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link  p-15" id="driver-tab" data-toggle="tab" href="#electronic" role="tab" aria-controls="home" aria-selected="false">Appliances</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link  p-15" id="driver-tab" data-toggle="tab" href="#automobile" role="tab" aria-controls="home" aria-selected="false">Automobile</a>
                    </li>

                </ul>
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show active" id="live" role="tabpanel" aria-labelledby="live-tab">
                        <div class="d-flex  row p-20 justify-content-between">

                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/monitor.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Tv</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/monitor.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Tv</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>





                        </div>
                    </div>
                    <div class="tab-pane fade" id="past" role="tabpanel" aria-labelledby="past-tab">
                        <div class="d-flex  row p-20 justify-content-between">
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="drivers" role="tabpanel" aria-labelledby="past-tab">
                        <div class="d-flex  row p-20 justify-content-between">
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="electrical" role="tabpanel" aria-labelledby="past-tab">
                        <div class="d-flex  row p-20 justify-content-between">
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="electronic" role="tabpanel" aria-labelledby="past-tab">
                        <div class="d-flex  row p-20 justify-content-between">
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="automobile" role="tabpanel" aria-labelledby="past-tab">
                        <div class="d-flex  row p-20 justify-content-between">
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/bed.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Bed</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/cabinet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Table</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>
                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/closet.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Cupboard</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                            <div class="simple-card category-cards col-sm-2">
                                <div class="card-title">Furniture</div>
                                <div class="card-body">
                                    <img src="./assets/images/chair.svg">
                                </div>

                                <div class="card-footer d-felx  justify-content-between">
                                    <div class="item-name">Chair</div>
                                    <div class="actions justify-content-between">
                                        <i><img src="./assets/images/Icon material-remove-red-eye.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon material-edit.svg" alt="" srcset=""></i>
                                        <i><img src="./assets/images/Icon metro-bin.svg" alt="" srcset=""></i>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>


            </div>
        </div>
    </div>




</div>

@endsection
